...
const ApplyforJob = () => {
  ...
  const [uploadClicked, setUploadClicked] = useState(false); // State to track if upload button is clicked
  ...
  const handleUploadButtonClick = async (e) => {
    ...
    setUploadClicked(true); // Set upload button clicked
    try {
      ...
      const interval = setInterval(() => {
        setProgress(prevProgress => {
          const randomIncrement = Math.floor(Math.random() * 10) + 5; // Random increment between 1 and 10
      ...
      // Notification for successful extraction only when progress reaches 100%
      if (progress === 100) {
        setNotification('Skills extracted successfully.');
        setNotificationColor('green');
        setTimeout(() => {
                 ...
                 {uploadClicked && ( 
                  <div className="mb-4">
                    <div className="flex items-center justify-between">
                      <p className="text-lg font-semibold dark:text-gray-400">Parsing your skills...</p>
                      <p className="text-lg font-semibold dark:text-gray-400">{progress}%</p>
                    </div>
                    <div className="w-full h-3 bg-gray-200 rounded-full mt-2 overflow-hidden">
                      <div className={`h-full bg-green-500 transition-all ease-in-out duration-500`} style={{ width: `${progress}%` }}></div>
                    </div>
                  </div>
                )}
